...
const SECRET_PATTERNS: Array<{ pattern: RegExp; replacement: string }> = [
  ...
  { pattern: /\b(eyJ[a-zA-Z0-9_-]{20,}\.[a-zA-Z0-9_-]{20,}\.[a-zA-Z0-9_-]{20,})\b/g, replacement: '[REDACTED_JWT]' },

  // Generic key=value patterns
  { pattern: /(?:api[_-]?key|apikey|secret[_-]?key|access[_-]?token|auth[_-]?token|bearer|password|passwd|pwd)\s*[:=]\s*["']?([^\s"']{8,})["']?/gi, replacement: '[REDACTED_CREDENTIAL]' },
  ...
  // .env style secrets
  { pattern: /^[A-Z_]*(SECRET|TOKEN|PASSWORD|KEY|CREDENTIAL)[A-Z_]*\s*=\s*\S+/gm, replacement: '[REDACTED_ENV_VAR]' },
...
export async function distillSessions(): Promise<{ updated: string[]; created: string[]; skipped: string[] }> {
  const config = getConfig();
  ...
  for (const [normalizedName, pi] of insights) {
    ...
    if (issues.length > 0) {
      console.error(`[cortex] BLOCKED distill for "${normalizedName}": ${issues.join(', ')}`);
    ...
    const existingPath = entryMap.get(normalizedName)
      ?? entryMapNoDash.get(normalizedName.replace(/-/g, ''));
    ...
    try {
      if (existingPath) {
        ...
        if (mergedIssues.length > 0) {
          console.error(`[cortex] BLOCKED merge for "${normalizedName}": ${mergedIssues.join(', ')}`);
          skipped.push(normalizedName);
          continue;
        }
      ...
      console.error(`[cortex] Failed to distill project ${normalizedName}: ${err}`);
    }
