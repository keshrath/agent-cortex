...
export class VectorStore {
   ...
   * @param dbPath - Path to the SQLite database file. Defaults to `{claudeDir}/cortex-vectors.db`.
   */
  constructor(dbPath?: string) {
    this.dbPath = dbPath ?? join(getConfig().claudeDir, "cortex-vectors.db");
  }
  ...
  private init(dimensions: number): void {
    ...
    try {
      ...
      console.error(`[cortex] Failed to initialize vector store: ${err}`);
      throw err;
    }
  }
  ...
  private loadVecExtension(): void {
    ...
    try {
      ...
      console.error(
        `[cortex] sqlite-vec extension not available, vector search disabled: ${err}`,
      );
  ...
  private ensureVecTable(dimensions: number): void {
    ...
    try {
      ...
      console.error(`[cortex] Failed to create vec0 table: ${err}`);
      this.vecAvailable = false;
    }
  }
  ...
  private setMetaValue(key: string, value: string): void {
    ...
    try {
      ...
      console.error(`[cortex] Failed to set meta value ${key}: ${err}`);
    }
  }
  ...
  private ensureInit(dimensions?: number): void {
    ...
    if (dims === null) {
      throw new Error("[cortex] Cannot initialize vector store without known dimensions");
    }
    this.init(dims);
  }
  ...
  upsert(entries: VectorEntry[]): void {
    ...
    try {
      ...
      const deleteBySourceEmbed = this.db.prepare(
        "SELECT id FROM embeddings WHERE source_id = ?",
      );
      ...
      console.error(`[cortex] Vector upsert failed: ${err}`);
    }
  }
  ...
  private searchInternal(
    ...
    if (!this.vecAvailable) {
      console.error("[cortex] Vector search unavailable — sqlite-vec not loaded");
      return [];
    }
    ...
    try {
      ...
      console.error(`[cortex] Vector search failed: ${err}`);
      return [];
    }
  }
  ...
  deleteBySource(sourceId: string): void {
    ...
    try {
      ...
      console.error(`[cortex] Delete by source failed: ${err}`);
    }
  }
  ...
  hasEmbeddings(sourceId: string): boolean {
    ...
    try {
      ...
      console.error(`[cortex] hasEmbeddings check failed: ${err}`);
      return false;
    }
  }
  ...
  wipe(): void {
    ...
    try {
      ...
      console.error(`[cortex] Wipe failed: ${err}`);
    }
  }
  ...
  stats(): VectorStoreStats {
    ...
    try {
      ...
      console.error(`[cortex] Stats query failed: ${err}`);
      return empty;
    }
  }
  ...
  close(): void {
    try {
      ...
      console.error(`[cortex] Failed to close vector store: ${err}`);
    } finally {
      this.db = null;
      this.initialized = false;
      this.currentDimensions = null;
