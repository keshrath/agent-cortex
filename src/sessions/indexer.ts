...
export async function backgroundIndex(): Promise<void> {
  const provider = await getEmbeddingProvider();
  if (!provider) {
    console.error('[cortex] No embedding provider available, skipping background index');
    return;
  }
  ...
  if (wiped) {
    console.error('[cortex] Embedding provider changed, wiped existing vectors');
  }
    ...
    for (const entry of entries) {
      try {
        ...
        if (indexed % 10 === 0 || indexed === total) {
          console.error(`[cortex] Indexed ${indexed}/${total} knowledge entries...`);
        }

        await new Promise(r => setTimeout(r, 100));
      } catch (err) {
        console.error(`[cortex] Failed to index knowledge entry ${entry.path}: ${err}`);
      }
    }

    if (total > 0) {
      console.error(`[cortex] Knowledge indexing complete: ${indexed}/${total}`);
    }
  } catch (err) {
    console.error(`[cortex] Knowledge indexing failed: ${err}`);
  }
  ...
  try {
      ...
      for (const sess of sessions) {
        try {
          ...
          if (indexedSessions % 20 === 0 || indexedSessions === totalSessions) {
            console.error(`[cortex] Indexed ${indexedSessions}/${totalSessions} sessions...`);
          }

          await new Promise(r => setTimeout(r, 100));
        } catch (err) {
          console.error(`[cortex] Failed to index session ${sess.id}: ${err}`);
          indexedSessions++;
        }
      }
    }
    ...
    if (totalSessions > 0) {
      console.error(`[cortex] Session indexing complete: ${indexedSessions}/${totalSessions}`);
    }
  } catch (err) {
    console.error(`[cortex] Session indexing failed: ${err}`);
  }

  console.error('[cortex] Background indexing complete');

  // ── Auto-distill session insights into knowledge base ───────────────────
  const config2 = getConfig();
  if (config2.autoDistill) {
    try {
      console.error('[cortex] Starting auto-distillation...');
      const result = await distillSessions();
      const total = result.updated.length + result.created.length;
      if (total > 0) {
        console.error(`[cortex] Distilled ${total} project(s): ${result.updated.length} updated, ${result.created.length} created`);
      } else {
        console.error('[cortex] No new sessions to distill');
      }
    } catch (err) {
      console.error(`[cortex] Auto-distillation failed: ${err}`);
    }
  }
}
