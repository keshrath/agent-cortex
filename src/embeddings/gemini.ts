  ...
  private async requestEmbeddings(
    ...
    try {
      ...
      if (response.status === 429 && retry) {
        ...
        console.error(`[cortex] Gemini rate limited, retrying in ${delayMs}ms`);
        await sleep(delayMs);
      ...
      if (!response.ok) {
        ...
        console.error(
          `[cortex] Gemini embedding request failed (${response.status}): ${body}`,
        );
        return texts.map(() => []);
      }
      ...
      console.error("[cortex] Gemini embedding request error:", err);
      return texts.map(() => []);
    }
  }
}
