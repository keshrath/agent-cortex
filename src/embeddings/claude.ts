  ...
  private async requestEmbeddings(
    ...
    try {
      ...
      if (response.status === 429 && retry) {
        ...
        console.error(`[cortex] Voyage rate limited, retrying in ${delayMs}ms`);
        await sleep(delayMs);
      ...
      if (!response.ok) {
        ...
        console.error(
          `[cortex] Voyage embedding request failed (${response.status}): ${body}`,
        );
        return texts.map(() => []);
      }
      ...
      console.error("[cortex] Voyage embedding request error:", err);
      return texts.map(() => []);
    }
  }
}
