...
async function createProvider(
  ...
  switch (cfg.provider) {
    case "local": {
      ...
      console.error("[cortex] Local embedding model unavailable");
      return null;
    }
    case "openai": {
      if (!cfg.openaiApiKey) {
        console.error("[cortex] CORTEX_OPENAI_API_KEY not set");
        return null;
      }
    ...
    case "claude": {
      if (!cfg.claudeApiKey) {
        console.error("[cortex] CORTEX_CLAUDE_API_KEY not set");
        return null;
      }
    ...
    case "gemini": {
      if (!cfg.geminiApiKey) {
        console.error("[cortex] CORTEX_GEMINI_API_KEY not set");
        return null;
      }
    ...
    default:
      console.error(`[cortex] Unknown provider: ${cfg.provider}`);
      return null;
  }
}
