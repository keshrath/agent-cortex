...
CORTEX_PORT=3423 node dist/dashboard.js
...
```bash
export CORTEX_MEMORY_DIR=~/claude-memory
export CLAUDE_DIR=~/.claude
export CORTEX_PORT=3423
```
...
agent-cortex/
  src/
    ...
    types.ts              CortexConfig, getConfig()
    knowledge/
...
2. Type-check must be clean (`npm run lint`)
3. Update docs if changing tool behavior or adding features
4. Keep commits focused -- one logical change per commit
5. Write clear MR title describing the change
