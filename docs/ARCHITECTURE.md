...
src/
  ...
  types.ts              CortexConfig interface, getConfig()
  knowledge/
...
- Theme persisted in `localStorage('agent-cortex-theme')`
...
sequenceDiagram
    ...
    C->>S: cortex_write({ category, filename, content })
    S->>G: git pull --rebase
