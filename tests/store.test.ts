...
import os from 'os';
import { parseFrontmatter, listEntries, readEntry, writeEntry, deleteEntry, sanitizePath } from '../src/knowledge/store.js';

// ── Helper: temp directory ─────────────────────────────────────────────────

function makeTmpDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'cortex-store-test-'));
}
...
describe('listEntries', () => {
  ...
  it('lists .md files with frontmatter', () => {
    ...
    fs.writeFileSync(path.join(projectsDir, 'test.md'), '---\ntitle: Test\ntags: [a]\n---\nContent');
  ...
  it('filters by tag (case-insensitive)', () => {
    const notesDir = path.join(tmpDir, 'notes');
    fs.mkdirSync(notesDir, { recursive: true });
    fs.writeFileSync(path.join(notesDir, 'tagged.md'), '---\ntitle: Tagged\ntags: [TypeScript, React]\n---\n');
