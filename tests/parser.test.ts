...
function makeTmpDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'cortex-parser-test-'));
}
...
describe('extractMessages', () => {
  it('extracts user messages with string content', () => {
    const entries = [
      { type: 'user', timestamp: '2026-01-01T00:00:00Z', message: { role: 'user', content: 'Hello world' } },
    ];
  ...
  it('extracts user messages with object content (JSON stringified)', () => {
    const entries = [
      { type: 'user', timestamp: '2026-01-01T00:00:00Z', message: { role: 'user', content: { text: 'complex' } } },
    ];
    const msgs = extractMessages(entries);
    expect(msgs.length).toBe(1);
...
describe('getSessionMeta', () => {
  it('extracts full metadata', () => {
    const entries = [
      { type: 'user', timestamp: '2026-01-01T10:00:00Z', cwd: '/proj', gitBranch: 'main', message: { content: 'First message' } },
  ...
  it('returns N/A preview when no user messages', () => {
    const entries = [{ type: 'assistant', timestamp: '2026-01-01T10:00:00Z', message: { content: 'hi' } }];
    expect(getSessionMeta(entries).preview).toBe('N/A');
  });

  it('truncates preview to 200 chars', () => {
    const entries = [{ type: 'user', timestamp: '2026-01-01T00:00:00Z', message: { content: 'a'.repeat(500) } }];
